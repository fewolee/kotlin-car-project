...
class CarUpdateAdapter(
    ...
    override fun update(carUpdateInDto: CarUpdateInDto): CarUpdateOutDto {
        ...
        return carOutMapper.toCarUpdateOutDto(updatedCar,updateCarCategoryEntity(carUpdateInDto, updatedCar))
    ...
    private fun updateCarEntity(carUpdateInDto: CarUpdateInDto): CarEntity {
        ...
        // 자동차 id로 자동차 엔티티 조회
        val car = carRepository.findById(carUpdateInDto.id)
            .orElseThrow { IllegalArgumentException("해당하는 자동차 ID가 없습니다: ${carUpdateInDto.id}") }

        // 자동차 정보 수정
        car.updateInfo(modelName = carUpdateInDto.modelName,
    ...
    private fun updateCarCategoryEntity(carUpdateInDto: CarUpdateInDto, updatedCar: CarEntity): List<String> {
