...
class CarReadController(
    ...
    ): ResponseEntity<CarModel> = ResponseEntity.ok(carReadUseCase.getById(id))

    // 모든 자동차 정보 조회
    @GetMapping
    @Operation(summary = "모든 자동차를 조회하는 API입니다", description = "모든 자동차의 정보들을 조회합니다")
    fun getAll(): ResponseEntity<List<CarModel>> = ResponseEntity.ok(carReadUseCase.getAll())

    // 카테고리로 자동차 정보 조회
    @GetMapping("/category/{category}")
    @Operation(summary = "카테고리로 자동차를 조회합니다", description = "카테고리를 입력받아 해당 카테고리의 모든 자동차를 조회합니다")
    fun getByCategory(
        @PathVariable @NotNull(message = "카테고리는 필수입니다") category: String,
    ): ResponseEntity<List<CarModel>> = ResponseEntity.ok(carReadUseCase.getByCategoryName(category))
    ...
    fun getByDynamicQuery(
        @Valid @ModelAttribute req: CarInfoListRequest,
    ): ResponseEntity<List<CarModel>> = ResponseEntity.ok(carReadUseCase.getDynamicQuery(carInMapper.toCarInfoListInDto(req)))
}
